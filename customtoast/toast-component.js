...
var ToastCmp = (function () {
    ...
    ToastCmp.prototype.cbClick = function (role) {
        if (this.enabled) {
            this.dismiss(role || 'close');
        }
    };

    ToastCmp.prototype.obClick = function (role) {
        if (this.enabled) {
            this.dismiss(role || 'open');
        }
    };
    ...
    ToastCmp.decorators = [
        { type: Component, args: [{
                    ...
                    template: '<div class="toast-wrapper" ' +
						...
						'<button ion-button clear class="toast-button" *ngIf="d.showOpenButton" (click)="obClick(d.openButtonText)"> ' +
                        '{{ d.openButtonText || \'Yes\' }} ' +
                        '</button> ' +
                        '<button ion-button clear class="toast-button" *ngIf="d.showCloseButton" (click)="cbClick(d.closeButtonText)"> ' +
                        '{{ d.closeButtonText || \'No\' }} ' +
                        '</button> ' +
                        '</div> ' +
                        '</div>',
                    host: {
